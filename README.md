...
## Что нового (форк, 2026)

- **Godot 4.6.2** — совместимость, autoload `AppLogger`.
- **Вся библиотека Steam** — поиск по имени/AppID, имена из CSV + Store API.
- **Загрузка крупных модов** — staging в `%LOCALAPPDATA%`, robocopy, диалог прогресса с логом, перекодирование превью.
- **Workshop по играм** — список item только для выбранного AppID; смена игры очищает dropdown.
- **Удаление UGC** — item не «воскресает» из черновиков/локального реестра.
- **UI** — адаптивная вёрстка главного окна, вставка пути к папке, удаление файлов из списка загрузки.

Запуск: `.\start.ps1`

Подробный список версий: [CHANGELOG.md](CHANGELOG.md)
...
| UI | Фиксированная вёрстка | **Адаптивная** + подпись «Items for: …» |
| Workshop по играм | Общий список | **Фильтр по AppID**, очистка при смене игры |
...
- Список Workshop **привязан к выбранной игре** (AppID): при смене игры dropdown очищается; черновики и item не подмешиваются с других игр.
...
| `Scripts/ui/Main/workshop_scope_label.gd` | Подпись текущей игры в блоке Workshop |
